...
# 1. Setup
epochs_data = np.load(r"data\cyton_participant-pedro_real_test\ses-001\2026-03-09_15h56.06.997\eeg_trials.npy", allow_pickle=True) * 1e-6 
sfreq = 250
ch_names = ['EEG1', 'EEG2', 'EEG3', 'EEG4', 'EEG5', 'EEG6', 'EEG7', 'EEG8']
info = mne.create_info(ch_names=ch_names, sfreq=sfreq, ch_types=['eeg']*8)

# 2. Your specific trigger order from the PsychoPy CSV
# 1 = Classical, 2 = EDM
trigger_order = np.array([2, 2, 1, 2, 1, 1, 1, 2, 1, 2]) 
...
# 6. Group by Genre and Calculate Means
results = []
...
# 7. VISUALIZE: The Genre Connection Bar Chart
...
plt.title("Brainwave Power Comparison: Classical vs. EDM", fontsize=14)
